...
[Pycharm](https://www.jetbrains.com/pycharm/download/download-thanks.html?platform=windows) for Windows 专业版


开发注解:软件安装包,使用教程,专业版破解教程

**Linux**

[Vim](https://www.onlinedown.net/download/10013834?module=download&t=website)编辑器、 [Pycharm](https://www.jetbrains.com/pycharm/download/download-thanks.html?platform=linux) for Linux 专业版

或者在Windows系统 [VMware虚拟机实现Ubuntu(Linux)操作系统](https://blog.csdn.net/weixin_41805734/article/details/120698714)


#### 2. 工具

- [XMind](https://www.xmind.cn/zen/download/win64/) 思维导图
- [Typora](https://www.typora.io/#download) 笔记查阅
- [GitHub](https://github.com/) 项目空间
...
- **面向对象OO**:将变量与函数绑定到一起，分类进行封装，每个程序只要负责分配给自己的分类，这样能够更快速的开发程序，减少了重复代码	**谁来做**

- **面向过程**：根据业务逻辑从上到下写代码	**怎么做**,注重步骤与过程，不注重职责分工;如果需求复杂，代码会变得很复杂;开发复杂项目，没有固定的套路，开发难度很大！
...
+ 云计算：
  ...
  *操作系统管理、服务器运维自动化脚本、桌面软件、服务器软件（网络软件）、游戏开发、人工智能与机器学习、*
...
**标识符命名**：以下划线或者A-Z/a-z中字母开头，长度不受限，区分大小写，不能用关键字


```python
a=b=c=d='nice'	# 连续赋值
e,f='to','meet you'	# 拆包
# input 用户输入   print程序输出
age = input('请输入您的名字:')# 注意：不管用户输入的是什么，变量保存的结果都是字符串。
print(age, a, e, f, sep='+',end="-------------")	
# print(type(age))  查看变量对应的值的数据类型（变量没有数据类型、数据才有数据类型）<class 'str'>
'''
...
**不可变类型**：字符串str、数字、元组tuple(,)、字典的键值

**可变类型**：列表list['','']、字典dict{'':''}、集合set{,'',} 、内存地址:值变址不变
...
常用的序列结构:字符串str/列表list/元组tuple/字典dict/集合set

*Key查找数据、get获取数据、为已存在的键赋值以修改数据*
...
**注意**：*字符串不可变！所有的字符串相关方法，都不会改变原有的字符串，都是返回一个结果，在这个新的返回值里，保留了执行后的结果！*
  ...
  判断:startswith,
endswith,isalpha,isdigit,isalnum,isspace

  计算出现次数:count

  替换内容:replace

  切割字符串（切片仅用于字符串、列表、元祖）:split,rsplit,splitlines,partition,rpartition
...
#### 1. 常用的2/8/10/16之间的进制转换

*默认输入输出为十进制。八进制只能用0o开头；两整数相除结果是一个浮点数；！=表示不等于运算符；不支持反引号`*
...
//避免二者命名同名；若相同，局部函数变量名=数据：即定义了一个局部变量而不是修改了全局变量
...
| [eval()](https://www.w3school.com.cn/python/ref_func_eval.asp) | 评估并执行字符串里的Python代码                                       |
...
闭包（函数快+引用环境）：在内部函数里对外部作用域变量进行应用，此内部函数为闭包。闭包内默认（局部变量）不能修改外部变量：
...
可以记住遍历的位置的对象,迭代（遍历）访问集合元素，从头至尾不后退；
...
属性对应某个实例方法(self)参数：内部进行一系列的逻辑计算，最终将计算结果返回
...
一个进程最少一个线程，线程属于进程，线程开销小，不利于维护；进程相反
...
线程安全问题——互斥锁（即协调线程同步机制）：锁定/非锁定锁定状态下其他线程不可写入
```python
# 创建锁
...
Python与Excel、Numpy、Matplotlib、Pandas一众扩展实现数据分析与可视化
...
[1^]:> **面向对象OO**与面向过程的区别和联系:
将变量与函数绑定到一起，分类进行封装，每个程序只要负责分配给自己的分类，这样能够更快速的开发程序，减少了重复代码	**谁来做**

[2^]:在完成某一个需求前，首先确定职责 —— 要做的事情（方法）。根据职责确定不同的对象，在对象内部封装不同的方法（多个）.最后完成的代码，就是顺序地调用不同对象的相应方法
